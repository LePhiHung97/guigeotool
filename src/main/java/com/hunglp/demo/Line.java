...
class SimpleDrawCanvasWithFiles extends Canvas implements MouseListener, MouseMotionListener {
  ...
  void doSaveToFile(Frame parentFrame) {
     ...
     * out.println(lineCount); // The number of lines in the data array. for (int i = 0; i <
     * lineCount; i++) { // Write the data for each indifvidual line. out.print(lines[i].x1);
     * out.print(" "); out.print(lines[i].y1); out.print(" "); out.print(lines[i].x2);
     * out.print(" "); out.print(lines[i].y2); out.print(" "); out.print(lines[i].colorIndex);
     * out.println(); }
     * 
  ...
  /*
   * public void paint(Graphics g) { // Redraw all the lines. for (int i = 0; i < lineCount; i++) {
   * int c = lines[i].colorIndex; g.setColor(Color.BLACK);
  ...
  int startX, startY; // When the user presses the mouse button, the location of the mouse is stored
                      // in these variables.
  int prevX, prevY; // The most recent mouse location; a rubber band line has been drawn from
                    // (startX, startY) to (prevX, prevY)
  boolean dragging = false; // For safety, this variable is set to true while a drag operation is in
                            // progress.
